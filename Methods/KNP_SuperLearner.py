...
def super_learner_predictions(X, y, latitude, longitude, models, meta_model):
    ...
    df_meta_X.to_csv("KNP_NEW_predicted_fitted_values.csv", index=False)
    # Predict
    return meta_model.predict(meta_X)



data = pd.read_csv('prefire_knp_blr_grouped_standard_finaldata.csv')
...
data.to_csv("KNP_traindata.csv", index=False)
test_df.to_csv("KNP_testdata.csv", index=False)
...
# Save to a csv file
meta_data.to_csv("KNP_NEW_meta_data.csv", index=False)

print('Meta KNP Final', meta_X.shape, meta_y.shape)
# fit base models
...
df_fitted_values.to_csv("KNP_NEW_fitted_values.csv", index=False)
...
coefficients.to_csv('KNP_NEW_coefficients.csv', index=False)
...
pd.DataFrame(yhat).to_csv("KNP_NEW_X_val_pred_super_learner.csv", index=False)
