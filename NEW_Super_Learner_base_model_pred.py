...
def super_learner_predictions(X, y, latitude, longitude, models, meta_model):
    ...
    df_meta_X.to_csv("Kincade_NEW_predicted_fitted_values.csv", index=False)
    # Predict
    return meta_model.predict(meta_X)



data = pd.read_csv('prefire_kincade_finaldata.csv')
...
y_pre = data.iloc[:, 0]  # Labels

features = data.drop(data.columns[0], axis=1)  # Features
...
df = pd.concat([lat, lon, y_pre, df], axis=1)
df.columns = ['Lat', 'Lon', 'dnbr'] + list(df.columns)[3:]
...
test_df.to_csv("Kincade_NEW_X_test.csv", index=False)


latitude = data.iloc[:, 0].values
longitude = data.iloc[:, 1].values
y = data.iloc[:, 2].values  # Labels
X = data.drop(data.columns[[0, 1, 2]], axis=1).values

lat_val = test_df.iloc[:, 0].values
lon_val = test_df.iloc[:, 1].values
...
# Save to a csv file
meta_data.to_csv("Kincade_NEW_meta_data.csv", index=False)

print('Meta Kincade Final', meta_X.shape, meta_y.shape)
# fit base models
...
df_fitted_values.to_csv("Kincade_NEW_fitted_values.csv", index=False)
...
coefficients.to_csv('Kincade_NEW_coefficients.csv', index=False)
...
pd.DataFrame(yhat).to_csv("Kincade_NEW_X_val_pred_super_learner.csv", index=False)
